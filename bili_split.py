...
# ---- 常量（避免散落的 magic number）----
TITLE_TIMEOUT = 90      # 视频标题查询超时 (秒)
SUBS_TIMEOUT = 300      # 字幕下载超时 (秒)
MODEL_DL_TIMEOUT = 1800  # 模型文件下载超时 (秒)
OVERLAP_TOLERANCE = 0.5  # 分段重叠容忍 (秒)
COOKIE_EXPIRY = 20 * 365 * 86400  # cookie 有效期 (20 年)
PROXY_DEFAULT = "http://127.0.0.1:7890"  # YouTube 默认代理
...
def parse_cuts(spec):
    ...
    for i in range(1, len(pairs)):
        if pairs[i][0] < pairs[i - 1][1] - OVERLAP_TOLERANCE:
...
def write_netscape_cookies(path, cookie_str, domain=".bilibili.com"):
    ...
    exp = str(int(__import__("time").time()) + COOKIE_EXPIRY)
...
def fetch_subs(ytdlp, ffmpeg_bin, proxy, site, yt_client, args, tmp, lang):
    ...
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True,
                       encoding="utf-8", errors="replace", timeout=SUBS_TIMEOUT)
...
def fetch_title(ytdlp, url, ffmpeg_bin, proxy, site, yt_client="android"):
    ...
    try:
        r = subprocess.run(cmd, capture_output=True, text=True,
                           encoding="utf-8", errors="replace", timeout=TITLE_TIMEOUT)
...
def cmd_fetch(args):
    ...
    bases = [(f"https://hf-mirror.com/Systran/faster-whisper-{name}/resolve/main/", None),
             (f"https://huggingface.co/Systran/faster-whisper-{name}/resolve/main/", PROXY_DEFAULT)]
    ...
    for fn in files:
        ...
        for base, proxy in bases:
            ...
            try:
                ...
                with op.open(url, timeout=MODEL_DL_TIMEOUT) as r, open(dst, "wb") as f:
                    f.write(r.read())
...
def main():
    ...
    p_imp.set_defaults(func=cmd_import_cookies)
